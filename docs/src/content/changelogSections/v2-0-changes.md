...
- Repair and support ships can now equip a Nuclear Propulsion upgrade, gated behind the nuclear engine techs (+4 upgrade levels per tier), extending their range up to a nuclear ship's at max level (Issue #2236) -[NKO] Various improvements and fixes overall for the content and quality of life improvements
...
- [NKO] Added the Foreign Affairs Office (Second Economic Committee) arms-export mechanic: sell modern conventional arms to non-Western nations losing their wars and deploy Storm Corps intervention forces for political favour and hard currency. Each client permanently raises factory output and production efficiency, but raises international sanctions; lobby Russia or China to ease the pressure, and reach ten clients to unlock the Arsenal of the Multipolar World capstone
...
- Antarctica International System
...
- Added Event Horizon custom scenario:
  ...
  - Entire world forms megastates and alliances to combat the BETA, gaining whole new focus tree, decisions, events and HACS Combat Mechs units to fight them off
- [ENG] Added UK Air OOB, ground force are also reworked
- [JAP] Added Japan Air OOB and better GFX, rework Ground and Navy OOB, add starting production
- Removed generic eastern europe scientist Victor Kozin because of the too low quality portrait
- [POL] State Run Economy decision category no longer has slow decisions, instead it has quick to use buttons.
- [BUL] Changes some effects in the Expansion Branch to reflect the situation better (Credit to Natha4796BG)
- [TAT] IT branch in economic focus tree
- Reworked Muslim Brotherhood content: new sponsor/suppressor decisions (including a rescind-designation decision to reverse terrorist branding), covert operations, transnational opinion modifiers, and idempotent mutual-relations on government change
- [GEO] [SOO] [ABK] Rework of ground OOB of this 3 contry, add air OOB to Abkhazia and fix military focus for owners of DLC
- [CZE] Fix missing icons, add Air OOB, add L-29 Delfin and add the T-72M4CZ
- Add or Rework air OOB to every contry
- [SOV] [FRA] [NKO] [SOV] [AZE] [NKR] [CHE] [ARM] Air and Ground OOB Rework
- [SYR] [IRQ] [CHI] Rework of all OOB
- Add More Transport and training Plane with their GFX

#TODO: [!] There were 2 content tabs, I unified them into 1. Below this line is what was a the top, revert this change if it was intentional to have the 2, else delete this comment to confirm it was not - DROID
...
- Liberate now revives dead nations when a core-holder no longer exists, or spawns a new splinter state when no core-holder remains
- Added ~834 new terrain photo province modifiers across ~90 nations
- [HKG] Major Hong Kong content expansion: ~142-focus tree across 12 branches (handover, LegCo, housing, police force, triads, 1C2S crisis, democratic reform, infrastructure, healthcare, labour, financial extension, tourism) plus 4 mutually exclusive endgame paths (loyalist puppet, Greater HK Nation, British Wing, City-State of Resistance)
- [HKG] Added ~32 national spirits and ideas covering housing, policing, financial autonomy, capital flight, Model SAR status, and the Triad criminal mechanic
- [HKG] Added ~20 new country events covering housing crises, police order, Beijing reactions, capital flight, and the post-NSL aftermath
- [HKG] Replaced the legacy "Annexed Hong Kong" focus with "The Model SAR" — a prosperity bonus scoped to the puppet state and removed the moment HKG escapes CHI subjugation
- [HKG] Added LegCo seat-tracking and motion-resolution scripted system (functional for railway/XRL/electoral-reform votes); standard policy motion clocks deferred pending GUI buttons
- [HKG] Added Triad strength/evidence variable system with a dynamic political-power-cost modifier driven by focus tree and decision actions
- [HKG] Added a CHI-side decision event for the anti-extradition crisis so puppeting goes through Beijing's choice rather than being forced from HKG's scope
- [HKG] Added the China Naval Procurement Contracts mechanic: a decision-driven system with solicit/deliver/negotiate actions, a scripted GUI banner tracking standing, lifetime revenue, and up to 3 active contracts, plus a capstone reward
- [HKG] Reworked the focus tree, adding a domestic aircraft carrier branch and an expanded trade and offshore-industry branch
- Added two biofuel Special Projects under the Energy tag: early E10 Ethanol Blending (gated on fuel_refining) and advanced FlexFuel and Butanol Synthesis (gated on fuel_efficiency4, requires E10 as parent), with SP_UNLOCK_PROJECT tooltips on both prerequisite techs (Issue #568)
- Added 2000s Ecuadorian coup d'état
- Reworked province map of Angola to be more accurate to the IRL borders
- Added Makamba to Southern Burundi as it's controlled by the [CNDD-FDD] as of 2000
- Tamel Tigers now exists as a tag on game start at war with Sri Lanka
- [ENG] United Kingdom now as a focus to perform the Wonga Coup
- [PER] Added 'NOHED' brigade to Iran's OOB
- [HOL] Expanded the airforce, diplomacy, and ASML branches
- [HOL] Added a Geert Wilders VVD path that can split off into his own party
- [HOL] Expanded the Dutch defense industry branch
- [HOL] Reworked the Dutch space program mechanic
- [HOL] Added the IJstad land-reclamation megaproject
- [HOL] Added a Mocro Maffia cartel mechanic
- [HOL] Added an annual naval diplomacy voyage mission that sends a Dutch ship abroad each year
- Added Antarctica tab into International Systems (where UN, PMC and other such stuff is placed)
  - Every proper country gained Antarctic Treaty member national spirit
  - Countries who signed either CCAS or CCAMLR treaties before 2000 gained national spirit with that treaty signed
  - With Antarctica comes Research Station architect system
