...
| **validate_gfx_references.py**        | Sprite names in `.gui` and scripted-GUI files are defined in `interface/*.gfx`; English `£name` loc refs that match a sprite only case-insensitively (ERROR — no icon on Linux); one name defined twice, and names differing only in case (WARNING). Opt-in: `--report-unused` (sprites defined but never referenced; engine-resolved equipment, tech, focus-filter and vanilla-override icons plus loc `£name` refs count as used). `MD_GFX_HIDE_UNUSED=1` drops just the orphan list from that run |
