...
- [RAJ] Added new Naxalite-Maoist insurgency mechanics ([#330])
...
- 9 new events: PWG-MCC Merger, Major Maoist Attack, Mass Surrender, Leadership Elimination, Tribal Displacement Protests, Operation Green Hunt Success, Naxal-Free Milestone, Dantewada Ambush (2010), Sukma Attack (2017)
- 5 new national ideas: Red Corridor Crisis, Operation Green Hunt Active, SAMADHAN Doctrine, Naxal-Free India, Salwa Judum Active
- [NIG] Added Niger Delta Revolt content ([#381])
...
- New decisions: Joint Task Force Operations, Negotiate with Militants, Implement Amnesty, Oil Revenue Sharing Agreement, Environmental Cleanup, Request International Mediation
- 20+ new events: MEND Formation (2006), Oil Platform Attacks, Foreign Worker Kidnappings, Pipeline Sabotage, International Pressure, Presidential Amnesty Program (2009), Militants Surrender, Amnesty Failure, Niger Delta Avengers Emergence (2015-16), State of Emergency, Crisis Resolution
- 10 new national ideas: Niger Delta Crisis (3 tiers), Amnesty Program, Failed Amnesty, Niger Delta Avengers, NDDC Reformed, Oil Revenue Sharing, Joint Task Force, Crisis Resolved
- Historical accuracy: 33% oil reduction during peak conflict, 2009 amnesty mechanics
- [SOV] Added decision for Russia to transform CSTO into offensive alliance ([#269])
...
- [GENERIC] Renamed North American formable from "United States of North America" to "North American Federation" ([#268])
- [BUL] Enhanced "Embracing Orthodoxy" focus to add Anti-LGBTQ Stance national spirit ([#284])
...
- If another coalition member is the larger party they become the ruling party and the others shift to the coalition to show popularity shifts ([#692])
...
- [RAJ] Added AI behavior for handling Naxalite-Maoist insurgency ([#330])
...
- Improved AI decision-making for Libyan tribal mechanics to prevent civil wars ([#153])
- [LBA] Improved AI handling of Gaddafi family stability mechanics ([#231])
...
- Fixed AI investment logic stacking all factories in single best region ([#114])
...
- Increased Nuclear Reactor base power gain from 2 GW to 5 GW
- Increased advanced building resource costs (tungsten 2→3, chromium 1→2)
...
- [RAJ] Complete rework of Naxalite-Maoist insurgency system ([#330])
...
- Fixed F-35 program still requiring NATO/MNNA status when opened to the world ([#286])
- Fixed coalition party removal exploit that allowed gaining infinite political power by spamming remove decisions ([#285])
...
- Added basic SAM technology (SAM and SAM0) to 34 countries that had SAM systems by 2000 with systems introduced no later than 1965: Albania, Algeria, Armenia, Austria, Azerbaijan, Belarus, Belgium, Bulgaria, Cuba, Czech Republic, Denmark, Estonia, Finland, Georgia, Greece, Hungary, Iraq, Kazakhstan, Latvia, Lithuania, Malaysia, Moldova, Netherlands, Norway, Romania, Serbia, Slovakia, Syria, Switzerland, Thailand, Turkmenistan, Uzbekistan, Vietnam, South Africa
- Added pre-existing intelligence agency upgrades for major powers reflecting their Y2K capabilities ([#120]):
...
- "International Aid" national spirit for Kosovo providing weekly income from UNMIK, USAID, and EU aid programs ([#230])
...
- CUB AI should no longer try bankrupting itself via focus tree
...
- Missile stat differentiation: later ICBM and IRBM tiers now have meaningful progression ([#115])
...
- SAM missile rebalance: significantly improved cost-effectiveness relative to static AA ([#113])
...
- [FRA] Removed commented-out MIRV tech assignments from France history file
- [SMA] Added declaration of friendship opinion modifier with Croatia to country history
- [SMA] Added level 1 bunker to province 13290 in state 948
- [SMA] Renamed division template from "Battaglione Di Polizia" to "The Gendarmerie"
- [BUL] Updated Bulgaria's starting setup to use dynamic modifiers for economy and army penalties
- Split the Naval Technologies into two tabs. Naval Hulls and Naval Equipment.
- [ISR] Added template with naval special forces for Isarel and unit's template changed (A more accurate representation of the Israeli special forces "Unit 13 'Shayetet'")
- [ISR] More accurate representation of israeli submarines: added INS Tanin (based on german Type 206 Class)
- [ISR] More accurate representation of israeli submarine deals: usualy Israel byus 3 submarines and Bibi's deal is also for 3 submarines. Both deals are about upgraded Dolphin class submarines
- [ISR] Proper representation of israeli corvettes: added templates for Sa'ar 4.5 Aliya Class and Sa'ar 4.5 Hetz Class and ships by those templates
...
- Added 'Focus Overlays' for political focuses in the Iranian tree
- Removed over 4,700 unused missile model GFX entries from MD_countrymissilesview.gfx
- Removed over 3,300 unused technology icon GFX entries from Technologies.gfx
- Moved deprecated missile UI graphics to resources/deprecated-missile-graphics/
- Moved unused technology icons to resources/deprecated-tech-graphics/
- [GUY] Standardized Guyana's leader portrait in 2000
- Added 3D models for the HACS mechs, BETA units, Defense Line wall, and Chimera Portal
- [ZOM] Added Zombie Spawn Location game rule with India, Central Africa, East Asia, and Multiple Locations options ([#205])
- [ZOM] Added Zombie Spawn Delay game rule with No Delay, 1 Year, 2 Year, and 3 Year countdown options ([#205])
- [ZOM] Added Anti-Zombie Coalition toggle game rule to enable or disable coalition formation ([#205])
...
- [MNT] Removed 3 unused custom tooltip keys for the Port of Bar dynamic modifier (MNT_port_bar_start_tt, MNT_port_bar_change_tt, MNT_port_bar_pirate_tt)
...
- Fixed all 148 missing or placeholder descriptions in the Georgian focus tree ([#149])
...
- Removed version number suffixes from all keys in research_l_english.yml
- Added localisation for BOS_aggressive_nation national spirit
...
- [GER] Added 15 missing generic dynamic modifier tooltip keys for army, navy, and air modifiers ([#790])
...
- Added new states for Philippines, Solomon Isles, Iran and Greenland for upcoming content
- Fixed over a dozen improperly placed Urban tiles (Houston, Tyumen, San Marino, Bratislava, etc) and set them to urban tiles
...
- Replaced inline every_country, random_country with more performance friendly alternatives where applicable
...
- Further optimized the AI strategies to stabilize the ticks and reducing redundant AI strategies
...
- Removed set_all_missile_tech scripted effect from on_startup to improve game start load time
- Added scripted triggers controls_a_state_with_hydroelectric and controls_a_state_with_geothermal to replace inline any_controlled_state calls in focus and AI conditions
- Extracted duplicate AA defence calculation into reusable scripted effect (calculate_raid_aa_defense)
- Extracted duplicate plane loss finalization into reusable scripted effect (finalize_raid_plane_losses)
- Extracted duplicate raid availability and show_target triggers into 6 reusable scripted triggers
- Refactored influence coup GUI to use is_top_fifty_influencer scripted trigger instead of inline variable checks
...
- Updated attack helicopter tech bonus to use CAT_atk_heli instead of CAT_heli
...
- [GER] Fixed Germany civil war being triggered too easily for the AI: raised ai_will_do base from 1 to 20 on the three BfV crisis purge decisions (GER_purge_banks, GER_purge_government_institutions, GER_purge_military) so the AI takes them within the 100-day mission window; also fixed NOT AND trap in their available blocks that allowed multiple purge decisions to be active simultaneously ([#1046])
- [ALG] Fixed Su-30 purchase focus not granting any aircraft due to invalid variant name "Su-30MKA"; corrected to "Su-30" ([#1025])
- [ALG] Fixed division-spawning focuses for special operations and marine forces silently failing because the "104th Operational Maneuvers Regiment RMO" template was never defined; added inline template creation with has_template guard ([#1025])
...
- Fixed generic fire control modules levels 4-6 being inaccessible on non-destroyer hulls due to wrong module category ([#987])
- [HLS] Removed Vatican City starting unit that immediately disbanded due to insufficient manpower (793 state manpower vs. 2,250 required for 3 infantry battalions) ([#872])
- [ENG] Fixed typo ENG_elisabeth_ruling_monarch → ENG_elizabeth_ruling_monarch in country history, which blocked completion of the Queen's Family focus tree branch ([#894])
- [SWE] Replaced attack_helo_bat with L_Air_assault_Bat in the NSB Helikopterflottiljen template; Sweden lacks attack helicopter tech in 2000 causing "no variant found for heavy_tank_chassis" log errors on every game start ([#822])
- [SOV] Added missing russia.3 and russia.4 events; russia.10 (Belarus integration choice) deferred the actual annexation/puppeting to russia.3 via effect_tooltip, so without it Belarus was never integrated despite the player accepting; russia.4 is the Belarus-refuses notification ([#824])
- [ENG] Restored missing experience_gain_submarine_training_factor modifier to the Andrew Naval Heritage decision ([#881], #452)
- Fixed cyber warfare operation flags not clearing when target nation reference was invalid (0 or -1); added guard to prevent scoping into uninitialized country slots ([#851])
...
- Fixed migration rate displaying as -213412341234% due to negative overflow when stacked modifiers reduced the multiplier to near-zero ([#783])
...
- Fixed terrorist_menace opinion modifier staying permanently even after a country changes away from Salafist government ([#399])
- [ENG] Fixed Nigeria operation granting oil resource rights on Lagos (state 334) instead of Biafra/Niger Delta (state 332) ([#657])
- [NKO] Fixed Russia declining the Strategic Partnership Treaty still granting the friendship idea to NKO, unlocking all Russian corporation decisions regardless of Russia's choice ([#233])
...
- Fixed PMC self-hire exploit where hiring a domestic PMC refunded the cost back to the player via the payment event ([#490])
- Fixed VTB PMC mission clearing the wrong flag (sberbank_yes instead of vtb_yes), preventing VTB from being rehired
- [SOM] Fixed somalia.3 event causing SNA to annex itself during unification — scoped change_tag_from and annex_country into SOM to fix ROOT scope mismatch ([#812])
...
- [ENG] Fixed commonwealth hegemony variables (commonwealth_total_gdp, commonwealth_total_army) never being reset before monthly accumulation, causing them to grow infinitely and making economic/military hegemony decisions permanently unavailable ([#657])
- Fixed futuristic capital fire control modules using orphan category, making them unplaceable on any ship hull ([#689])
- Fixed influence monthly exploit - auto-influence now applies cooldown on activation to prevent abuse of late-month toggle ([#378])
- Fixed AI investment proposals intermittently targeting wrong states by adding a pending-proposal guard flag to prevent the weekly pulse from overwriting staging variables before the target responds ([#227])
- Fixed investment exploit allowing players to bypass treasury checks while game is paused by adding runtime validation ([#374])
- Fixed unescaped quotes in raid event descriptions (MD_raid.2, MD_raid.4, MD_raid.5)
- [JAP] Fixed missing interface files for Japan in the Tank Designer - created new \_jap.gui files for all tank chassis types using USA layout ([#349])
- Fixed ship experience gain modifiers not showing ship type in tooltips ([#354])
...
- Fixed broken variant upgrade for Artillery 2005 by renaming non-NSB artillery technologies to avoid name collision with equipment ([#334])
- [BLR] Fixed factory locations in Belarus focuses - factories now build in correct provinces ([#236]):
...
- Added missing intelligence agency advisors for spymaster role ([#110])
- Fixed SAM missile icons not matching their equipment variants in deployment UI ([#218])
- Fixed Linux runtime crashes caused by parsing errors ([#176])
- Fixed configurable display showing 0 for all resources and added microchips/composites support ([#288])
...
- [ARM] Fixed Active Diplomacy focus auto-bypassing when in any faction instead of only when faction leader ([#203])
- [ARM] Fixed Embrace Left-Wing Origins focus not adding labour_unions if Bosses Elimination was completed first ([#203])
- Renamed "special forces" subdoctrine directory to "special_forces" to fix Linux path parsing
- Fixed NATO faction being disbanded when USA (or any faction leader) leaves NATO - leadership now transfers to another member ([#94])
- Fixed investment system allowing AI to offer investments for buildings when pending projects would already max out capacity ([#99])
- Fixed the Strv 121 tank model showing invisible due to missing entity definition ([#139])
- Fixed Big Ben and other landmark buildings not displaying due to missing GFX sprite definitions ([#140])
...
- Completely refactored Afghanistan Almond/Pomegranate investment mechanics to fix infinite money exploit ([#156])
...
- [PER] Fixed Kermanshah's Oxygen mission checking wrong province (12773 instead of city province 16155) ([#234])
...
- [PER] Fixed "General Max Army Size" modifier from Concessions to the Army focus not working ([#235])
- [PER] Added distinct color to Axis of Resistance faction to differentiate from CSTO on F10 map mode ([#342])
...
- [HOL/LIC] Fixed missing portrait texture errors ([#333])
- HOL: Fixed case mismatch in Wim Kok portrait reference (wim_kok.dds → Wim_Kok.dds)
- [PER/SER/BOL] Fixed create_unit parsing errors ([#331])
...
- [BUL] Fixed border training event having no valid options during civil wars ([#332])
...
- [ARM] Fixed the ARM_has_artsakh_idea not being properly removed when completing the focus ([#203])
...
- Fixed encryption/decryption techs being set unconditionally in country history files; they are now gated behind NOT having La Resistance DLC ([#608])
...
- Fixed your economic aid being evaporated when the target nation rejected the economic aid ([#680])
