...
Key state: `global.current_ga_vote_type/nation`, `current_ongoing_ga_vote`, `un_ga_vote_cooldown` (50d GA / 20d SC), `global.un_2_3_number` (GA yes votes required, ceil of 2n/3), and `global.un_sc_required_yes` (SC yes votes required, ceil of 60% of current Council membership). Both thresholds are recomputed when their chamber starts a vote.
...
- **A veto is a flag, not a tally.** `has_passed_sc_vote` is "no `security_council_veto` AND at least `global.un_sc_required_yes` yes votes". The threshold is ceil(60% of the current Council), preserving 9 of 15 while scaling with permanent-member expansion. The manual and automatic voting paths set the flag for a permanent member's "no"; `sc_sway_direction_effect` recomputes it from all current permanent no-voters after moving a vote. Never infer it from the total no tally.
...
- GA type 22 approval does not add `p5_member` directly. It calls `unsc_finalize_permanent_seat_approval`; if an unrelated SC vote is live, promotion waits until that vote finishes or aborts. `unsc_promote_permanent_member` is the only non-startup ascension seam: it removes elected-seat candidacy/confirmation and queued type-6 votes, backfills an occupied elected slot 0-9, then grants `p5_member`.
...
The weekly queue pulse validates that the stage flag still has a matching live or queued type-8/type-22 vote. Missing state fails the application and releases the lock. An approved applicant waiting behind a live SC vote is promoted as soon as that vote finishes or aborts.
