...
def _build_state(state_id, manpower, owner, prod, buildings, resources=None,
                 category="state_02", renewables=None, hydro=None, geo=None,
                 cap_var=None):
    ...
    if cap_var is not None:
        lines.append(f"\t\tset_variable = {{ state_renewable_capacity_factor_modifier_var = {cap_var} }}")
    if hydro is not None:
        lines.append(f"\t\tset_variable = {{ hydroelectric_energy_production_var = {hydro} }}")
    if geo is not None:
        lines.append(f"\t\tset_variable = {{ geothermal_energy_production_var = {geo} }}")
...
def full_mini_repo(mini_repo, monkeypatch):
    ...
    write_text(
        ...
        _build_state(
            state_id=10, manpower=2_000_000, owner="TST", prod=700,
            buildings={"industrial_complex": 6, "arms_factory": 3},
            category="state_03", renewables=True, cap_var=1.2, hydro=0.5, geo=0.0,
        ),
    )
    write_text(
        mini_repo / "history" / "states" / "20-B.txt",
        _build_state(
            state_id=20, manpower=1_000_000, owner="TST", prod=400,
            buildings={"industrial_complex": 1, "offices": 2, "renewable_energy_infra": 4},
            category="state_01", renewables=True, cap_var=0.8, hydro=0.0, geo=0.3,
        ),
    )
    ...
    write_text(
        ...
        _build_state(
            state_id=30, manpower=500_000, owner="NUK", prod=600,
...
class TestCompositeSeeds:
    def test_load_seed_values_skips_header_comment_blank(self, import_pre_place, full_mini_repo):
        mod = import_pre_place
    ...
    def test_parse_composite_seeds_includes_one_per_country(self, import_pre_place, full_mini_repo):
        mod = import_pre_place
...
class TestEnergyConsumption:
    ...
    def test_population_and_building_contributions(self, import_pre_place, full_mini_repo):
        mod = import_pre_place
    ...
    def test_population_modifier_applied(self, import_pre_place, full_mini_repo):
        ...
        state = {
            "manpower": 1_000_000, "buildings": defaultdict(int),
        }
        base = mod.energy_consumption([state], {"pop_energy_use_multiplier": 0.0, "energy_use_multiplier": 0.0}, gdpc=10.0)
        boosted = mod.energy_consumption([state], {"pop_energy_use_multiplier": 1.0, "energy_use_multiplier": 0.0}, gdpc=10.0)
...
class TestEnergySupply:
    def test_renewable_and_hydro_and_geo(self, import_pre_place, full_mini_repo):
        ...
        supply = mod.energy_supply_non_fossil(states, {"energy_gain_multiplier": 0.0}, has_nuclear=False)
    ...
    def test_nuclear_contribution(self, import_pre_place, full_mini_repo):
        ...
        for f in sorted((full_mini_repo / "history" / "states").iterdir()):
            ...
            if s["owner"] == "NUK":
                nuk_states.append(s)
        supply = mod.energy_supply_non_fossil(nuk_states, {"nuclear_energy_generation_modifier": 0.0}, has_nuclear=True)
        # 2 reactors * 4 GW each = 8 GW.
        assert supply == pytest.approx(8.0)

    def test_nuclear_skip_when_has_nuclear_false(self, import_pre_place, full_mini_repo):
        mod = import_pre_place
...
class TestDistribute:
    def test_zero_count_returns_empty(self, import_pre_place, full_mini_repo):
        ...
        states = [mod.parse_state_with_category(str(f)) for f in sorted((full_mini_repo / "history" / "states").iterdir())]
    ...
    def test_weighted_distribution_favours_industrial_state(self, import_pre_place, full_mini_repo):
        mod = import_pre_place
    ...
    def test_composite_weight_skips_states_without_arms_or_dockyards(self, import_pre_place, full_mini_repo):
        mod = import_pre_place
...
class TestInjectBuilding:
    def test_injects_into_existing_buildings_block(self, import_pre_place, mini_repo):
        ...
        assert ok, msg
        text = (mini_repo / "history" / "states" / "100.txt").read_text(encoding="utf-8")
    ...
    def test_replace_existing_top_level_entry(self, import_pre_place, mini_repo):
        ...
        assert ok, msg
        text = (mini_repo / "history" / "states" / "101.txt").read_text(encoding="utf-8")
    ...
    def test_creates_buildings_block_when_missing_after_owner(self, import_pre_place, mini_repo):
        mod = import_pre_place
        write_text(
        ...
        assert ok, msg
        text = (mini_repo / "history" / "states" / "102.txt").read_text(encoding="utf-8")
        assert "buildings = {" in text
        assert "composite_plant = 1" in text

    def test_creates_buildings_block_when_no_owner_line(self, import_pre_place, mini_repo):
        mod = import_pre_place
        write_text(
        ...
        assert ok
        text = (mini_repo / "history" / "states" / "103.txt").read_text(encoding="utf-8")
        assert "composite_plant = 1" in text
...
class TestInjectReactorStockpile:
    def test_inserts_after_enrichment_array(self, import_pre_place, mini_repo):
        ...
        ok, msg = mod.inject_reactor_stockpile(
            str(mini_repo / "history" / "countries" / "RTT - Test.txt"), 5000, set_flag=True
        )
        assert ok, msg
        text = (mini_repo / "history" / "countries" / "RTT - Test.txt").read_text(encoding="utf-8")
        ...
        assert text.index("var_reactor_material_stockpile") < text.index("overall_productivity")
    ...
    def test_idempotent_replace(self, import_pre_place, mini_repo):
        ...
        ok, msg = mod.inject_reactor_stockpile(
            str(mini_repo / "history" / "countries" / "RTT - Test.txt"), 7000, set_flag=True
        )
        assert ok, msg
        text = (mini_repo / "history" / "countries" / "RTT - Test.txt").read_text(encoding="utf-8")
        ...
        assert text.count("set_country_flag = enabled_nuclear_reactor_fuel_production") == 1

    def test_anchor_is_overall_productivity_when_no_enrichment_array(self, import_pre_place, mini_repo):
        mod = import_pre_place
        write_text(
        ...
        ok, _ = mod.inject_reactor_stockpile(
            str(mini_repo / "history" / "countries" / "NOP - Test.txt"), 1000, set_flag=False
        )
        assert ok
        text = (mini_repo / "history" / "countries" / "NOP - Test.txt").read_text(encoding="utf-8")
    ...
    def test_no_anchor_returns_false(self, import_pre_place, mini_repo):
        ...
        ok, msg = mod.inject_reactor_stockpile(
            str(mini_repo / "history" / "countries" / "EMP - Test.txt"), 1000, set_flag=True
        )
        assert not ok
...
class TestPipeline:
    ...
    def test_compute_placements_skips_paper_tags(self, import_pre_place, full_mini_repo):
        mod = import_pre_place
        data = mod.gather_country_data()
        composite_seeds = mod.parse_composite_seeds()
    ...
    def test_apply_plan_dry_run_returns_count(self, import_pre_place, full_mini_repo):
        ...
        before = (full_mini_repo / "history" / "states" / "10-A.txt").read_text(encoding="utf-8")
        n = mod.apply_plan(plan, data, write=False)
        after = (full_mini_repo / "history" / "states" / "10-A.txt").read_text(encoding="utf-8")
        assert before == after
        assert n >= 1
    ...
    def test_apply_plan_write_modifies_files(self, import_pre_place, full_mini_repo):
        ...
        a_text = (full_mini_repo / "history" / "states" / "10-A.txt").read_text(encoding="utf-8")
        nuk_text = (full_mini_repo / "history" / "countries" / "NUK - Test.txt").read_text(encoding="utf-8")
    ...
    def test_apply_plan_only_filter(self, import_pre_place, full_mini_repo):
        ...
        composite_seeds = mod.parse_composite_seeds()
        plan = mod.compute_placements(data, composite_seeds)
        before_nuk = (full_mini_repo / "history" / "countries" / "NUK - Test.txt").read_text(encoding="utf-8")
        mod.apply_plan(plan, data, write=True, only={"TST"})
        after_nuk = (full_mini_repo / "history" / "countries" / "NUK - Test.txt").read_text(encoding="utf-8")
...
class TestCli:
    def test_dry_run_prints_table_and_does_not_write(self, import_pre_place, full_mini_repo, capsys, monkeypatch):
        mod = import_pre_place
        ...
        for f in (full_mini_repo / "history" / "states").iterdir():
            ...
            assert "fossil_powerplant = " not in text or text.count("fossil_powerplant = ") == 1

    def test_write_emits_count(self, import_pre_place, full_mini_repo, capsys, monkeypatch):
        mod = import_pre_place
    ...
    def test_top_truncates_table(self, import_pre_place, full_mini_repo, capsys, monkeypatch):
        mod = import_pre_place
        monkeypatch.setattr(sys, "argv", ["pre_place.py", "--dry-run", "--top", "1"])
        mod.main()
        out = capsys.readouterr().out
        data_lines = [l for l in out.splitlines() if l.startswith("TST") or l.startswith("NUK")]
        # At most one data row when --top 1.
        assert len(data_lines) <= 1

    def test_only_filter_emits_subset(self, import_pre_place, full_mini_repo, capsys, monkeypatch):
        mod = import_pre_place
