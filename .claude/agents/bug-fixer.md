...
- **Tautological OR in `ai_will_do`/`ai_chance`** — an `OR` covering all possible values of a trigger (e.g. both `is_historical_focus_on = yes` and `= no` in the same block) is always true; the modifier fires unconditionally. Remove the OR and fold the `add` value into `base`.
- **`threat` comparisons as percentages** — `threat` is a decimal 0.0–1.0. `threat > 10` or `threat > 40` are always false. Should be `threat > 0.10`, `threat > 0.40`, etc.
