...
CI runs `python3 tools/docs_checks/check_docs_hygiene.py` (from the repo root) against `docs/public/assets/images/`, `docs/public/assets/downloads/`, and `docs/src/assets/images/`. It fails if a tracked asset is never referenced from other docs sources using `/assets/images/...`, `@/assets/images/...`, or a `src/assets/images/...` path string. Remove unused files or add an explicit entry to `ALLOW_UNUSED_ASSETS` in that script only when keeping an unreferenced file is intentional.
