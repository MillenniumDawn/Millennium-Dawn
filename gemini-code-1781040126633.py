...
def generate_diff_string(str1, str2):
    ...
    for op, i1, i2, j1, j2 in matcher.get_opcodes():
        if op == 'equal':
            ...
            result1.append(f"{CLR_UNDERLINE}{CLR_DIFF}{str1[i1:i2]}{CLR_RESET}{CLR_MODDED}")
            result2.append(f"{CLR_UNDERLINE}{CLR_DIFF}{str2[j1:j2]}{CLR_RESET}{CLR_VANILLA}")
        elif op == 'delete':
            result1.append(f"{CLR_UNDERLINE}{CLR_DIFF}{str1[i1:i2]}{CLR_RESET}{CLR_MODDED}")
        elif op == 'insert':
            result2.append(f"{CLR_UNDERLINE}{CLR_DIFF}{str2[j1:j2]}{CLR_RESET}{CLR_VANILLA}")
...
def main():
    ...
    for key, entries in duplicates.items():
        ...
        if md_entry and vanilla_entry:
            ...
            print(f"  * Target Replace File:        {CLR_TARGET}localisation/english/replace/{target_file}{CLR_RESET} ({status})")
            print("-" * 70)
            
            while True:
                choice = input("Approve this migration? (y/n) or 'q' to quit: ").strip().lower()
                if choice in ['y', 'n', 'q']:
                    break
            ...
            if choice == 'q':
                ...
                if remove_key_from_file(md_file_path, key):
                    ...
                    print(f"--> [ERROR] Could not find the key in the file to delete it.\n")
            else:
    ...
    print("=" * 70)
    print(f"Process complete. Reviewed {processed_count}/{total_keys} keys. Approved & migrated {approved_count}.")
